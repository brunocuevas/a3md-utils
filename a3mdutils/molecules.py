...
def get_segment_population(segments: list, segments_ids: list, atomic_numbers: np.ndarray):
    segment_charges = []
    segments = np.array(segments)
    for i, segid in enumerate(segments_ids):
        ans = atomic_numbers[segments == i]
        sq = int(ans.sum()) - PDB_PROTEIN_CHARGES[segid]['charge']
        segment_charges.append(sq)
    return segment_charges


def get_segment_charge(segments_ids: list):
    # IMPORTANT NOTE: Charge is inverted, given that we use density with a positive sign.
    segment_charges = []
    for i, segid in enumerate(segments_ids):
        sq = -PDB_PROTEIN_CHARGES[segid]['charge']
        segment_charges.append(sq)
    return segment_charges
...
class Mol2(MolRepresentation):
    ...
    def get_seqment_charges(self):
        sidx = np.unique(np.array(self.segment_idx))
        q = []
        for i in sidx:
            mask = self.segment_idx == i
            q.append(self.charges[mask].sum().round())
...
class XYZFile(MolRepresentation):

    def __init__(self, filename):

        natoms, symbols, coords, meta = self.read(filename)
        ans = [get_atomic_number(i) for i in symbols]
        super(XYZFile, self).__init__(
            name=filename, units='angstrom', number_atoms=natoms,
            number_bonds=None, atomic_numbers=ans, coordinates=coords, charges=None,
            atom_types=None, atom_names=None, bonds=None, bond_types=None
        )
        self.metadata = meta
    ...
    def read(filename):
        ...
        for i in range(natoms):
            ...
            xyz[i, :] = [float(i) for i in line[1:]]

        return natoms, symbols, xyz, metadata
